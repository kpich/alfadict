"""Generate CC task files for quality-control decisions.

For each sampled wordform, writes a CCQCTask JSON file to
cc_tasks/pending/qc/. Forms that are already redirects, fully
morph-tagged, blocklisted, or already have a pending/done task file are skipped.

Usage:
    python -m alfs.update.refinement.generate_qc_tasks \\
...
from alfs.cc.models import CCQCTask, SenseInfo
...
def run(
    ...
    pending_dir = tasks_dir / "pending" / "qc"
    done_dir = tasks_dir / "done" / "qc"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate CC QC task files")
